...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("⏳ جاري تحليل ملف الإكسل وتحديث هيكل الموظفين بالكامل...")
        ...
        required_cols = ["الرقم الوظيفي", "الاسم", "رقم الهاتف الدولي"]
        if not all(col in df.columns for col in required_cols):
            await update.message.reply_text("❌ الملف المرفوع لا يحتوي على الأعمدة الأساسية المطلوبة:\n(الرقم الوظيفي، الاسم، رقم الهاتف الدولي).")
            if os.path.exists(file_path): os.remove(file_path)
            return

        existing_employees = {e.id: e for e in db.query(Employee).all()}
        uploaded_ids = set()
        ...
        for index, row in df.iterrows():
            if pd.isna(row["الرقم الوظيفي"]) or str(row["الرقم الوظيفي"]).strip() == "":
                continue
                
            raw_id = str(row["الرقم الوظيفي"]).strip()
            emp_id = raw_id.split('.')[0] if '.' in raw_id else raw_id
            
            if emp_id in uploaded_ids:
                continue
            uploaded_ids.add(emp_id)

            emp_name = str(row["الاسم"]).strip() if pd.notna(row["الاسم"]) else "موظف غير معروف"
            raw_phone = str(row["رقم الهاتف الدولي"]).strip() if pd.notna(row["رقم الهاتف الدولي"]) else ""
            ...
            if emp_role not in ["Admin", "Owner", "Employee"]:
                emp_role = "Employee"
            ...
            if emp_id in existing_employees:
                ...
                new_emp = Employee(
                    ...
                    role=emp_role,
                    encrypted_salary=encrypt_data("1200.0"),
                    vacation_balance=30
                )
                db.add(new_emp)
                inserted_count += 1

        # تعديل جوهري: بدلاً من الحذف المباشر الذي قد يفشل بسبب القيود، نقوم بتعطيل الحسابات
        # أو محاولة الحذف وإذا فشل نقوم بالتعطيل
        deleted_count = 0
        for old_id in existing_employees:
            if old_id not in uploaded_ids:
                try:
                    # محاولة حذف الموظف
                    emp_to_del = existing_employees[old_id]
                    db.delete(emp_to_del)
                    db.flush() # تجربة الحذف فوراً لاكتشاف أي خطأ في القيود
                    deleted_count += 1
                except Exception:
                    db.rollback() # تراجع عن الحذف الفاشل فقط
                    # إذا فشل الحذف بسبب القيود، نقوم بتعطيل الحساب
                    existing_emp = db.query(Employee).filter(Employee.id == old_id).first()
                    if existing_emp:
                        existing_emp.phone_number = f"disabled_{old_id}_{random.randint(100,999)}"
                        existing_emp.role = "Disabled"
                        deleted_count += 1

        db.commit()
        sessions[chat_id]["step"] = "AI_CHAT_MODE"
        await update.message.reply_text(
            f"✅ **تم تحديث واستبدال هيكل البيانات بنجاح!**\n\n"
            f"📥 حسابات جديدة مضافة: {inserted_count}\n"
            f"🔄 حسابات تم تحديثها: {updated_count}\n"
            f"🗑️ حسابات قديمة تمت إزالتها/تعطيلها: {deleted_count}"
        )

...
async def show_admin_menu(bot, chat_id):
    # 1. تم هنا فصل الأزرار إلى تصدير وإستيراد بشكل مستقل وتصحيح استدعاء المذكرات
    keyboard = [
        [
...
async def show_employee_menu(bot, chat_id, employee_name):
    ...
    await bot.send_message(chat_id=chat_id, text=2📋 لوحة الخدمات الذاتية للموظف:", reply_markup=InlineKeyboardMarkup(inline_keyboard))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إذا كان المالك أو المسؤول يفتح البوت، تظهر له لوحة التحكم أولاً
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الاستيراد يعمل فقط إذا كان المدير قد ضغط على زر إستيراد مسبقاً لحماية البيانات من الرفع العشوائي
    ...
    await update.message.reply_text("⏳ جاري قراءة، تنظيف واستيراد ملف البيانات الجديد...")
    try:
        ...
        for index, row in df.iterrows():
            if pd.isna(row["الرقم الوظيفي"]) or str(row["الرقم الوظيفي"]).strip() == "": continue
        ...
        logger.error(e)
        await update.message.reply_text("❌ حدث خطأ بنيوي أثناء معالجة خلايا الإكسل الحالية.")
    finally:
        db.close()
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🧼 ميزة تنظيف الشاشة والعودة لنقطة البداية (Start) من الصفر
    if query.data == "clear_and_restart":
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text="🧹 تم تنظيف الجلسة بنجاح.\n\n🔒 يرجى إدخال **رقم هاتفك** من جديد للدخول الموثق:",
    ...
    # 📤 تنفيذ أمر "تصدير" جلب البيانات المخزنة حالياً وإرسالها كملف إكسيل فوري
        ...
        with open(file_name, "rb") as f:
            await context.bot.send_document(chat_id=chat_id, document=f, filename=file_name, caption="📊 إليك ملف الإكسل الذي يحتوي على كافة البيانات الحالية المخزنة بنجاح.", reply_markup=InlineKeyboardMarkup(kb))
        os.remove(file_name)
        
    # 📥 تنفيذ أمر "إستيراد" تهيئة السيرفر لاستقبال ملف حديث من المشرف
    elif query.data == "admin_import_trigger":
        sessions[chat_id]["step"] = "WAITING_FOR_EXCEL_IMPORT"
        kb = [[InlineKeyboardButton("❌ إلغاء العملية", callback_data="back_to_admin_menu")]]
        await query.message.reply_text("📥 النظام جاهز الآن لاستقبال البيانات!\nيرجى القيام بسحب وإرسال (Upload) ملف الإكسيل المحدث بصيغة `.xlsx` مباشرة الآن في الشات هنا.", reply_markup=InlineKeyboardMarkup(kb))

    # 🗂️ إصلاح استجابة إدارة المذكرات والشكاوى بالكامل لتظهر بسلاسة
        ...
        if not reqs:
            await query.message.reply_text("✅ لا توجد طلبات إجازة معلقة حالياً في السجلات.")
        for r in reqs:
        ...
        if ticket:
            ...
            db.commit()
            await query.message.reply_text("✅ تم أرشفة وإغلاق المذكرة بنجاح.")

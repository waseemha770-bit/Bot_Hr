...
# 🔄 الدالة المحدثة كلياً لضمان الاستبدال الكامل بدون أخطاء بنيوية
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("⏳ جاري تفريغ قاعدة البيانات القديمة واستبدالها بالهيكل الجديد بالكامل...")
        ...
        if new_employees:
            # مسح البيانات السابقة بالكامل لتمكين ميزة الاستبدال النظيف
            db.query(Employee).delete(synchronize_session=False) 
            
            db.bulk_save_objects(new_employees)
            db.commit()
            
            sessions[chat_id]["step"] = "AI_CHAT_MODE"
            await update.message.reply_text(f"✅ **تم استبدال هيكل البيانات بالكامل بنجاح!**\n📊 إجمالي الموظفين النشطين حالياً في النظام: {len(new_employees)} موظف.")
        else:
            await update.message.reply_text("⚠️ فشل الاستيراد: الملف المرفوع لا يحتوي على أي بيانات موظفين صالحة.")

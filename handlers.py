...
# 🔄 الدالة النهائية المستبدلة والمحمية كلياً ضد الأخطاء البنيوية والقيود
    ...
    try:
        ...
        for old_id in existing_employees:
            if old_id not in uploaded_ids:
                try:
                    db.delete(existing_employees[old_id])
                    deleted_count += 1
                except Exception:
                    existing_employees[old_id].phone_number = f"disabled_{old_id}"
                    existing_employees[old_id].role = "Disabled"
                    deleted_count += 1
        ...
        await update.message.reply_text(
            ...
            f"🗑️ حسابات قديمة تمت إزالتها: {deleted_count}"
        )
        ...
        await update.message.reply_text("❌ حدث خطأ بنيوي أثناء استبدال خلايا الإكسل. يرجى مراجعة تنسيق الملف ومحاولة الرفع مجدداً.")
    finally:
        db.close()

...
# دالات عرض القوائم لتسهيل الرجوع إليها
async def show_admin_menu(bot, chat_id):
    ...
    await bot.send_message(
        chat_id=chat_id,
        text="⚙️ **لوحة التحكم الإدارية للموارد البشرية:**\n*(ملاحظة: لتحديث البيانات، قم برفع ملف الـ Excel الجديد مباشرة إلى الشات هنا)*",
...
# 📥 دالة استقبال وتحديث البيانات عن طريق رفع ملف Excel جديد للبوت
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    db = SessionLocal()
    current_user = db.query(Employee).filter(Employee.telegram_id == str(chat_id)).first()
    
    # السماح فقط للمالك أو المدير بتحديث قاعدة البيانات
    if chat_id != OWNER_CHAT_ID and (not current_user or current_user.role not in ["Owner", "Admin"]):
        await update.message.reply_text("❌ عذراً، لا تمتلك صلاحيات تحديث ملف النظام.")
        db.close()
        return

    doc = update.message.document
    if not doc.file_name.endswith('.xlsx') and not doc.file_name.endswith('.xls'):
        await update.message.reply_text("❌ يرجى رفع ملف بصيغة Excel فقط (تنتهي بـ .xlsx)")
        db.close()
        return

    await update.message.reply_text("⏳ جاري استقبال ملف الـ Excel وتحديث قاعدة البيانات...")
    ...
    try:
        ...
        # قراءة الملف عبر pandas
        df = pd.read_excel(file_path)
        
        # التأكد من وجود الأعمدة المطلوبة
        required_columns = ["الرقم الوظيفي", "الاسم", "رقم الهاتف الدولي", "الصلاحية"]
        if not all(col in df.columns for col in required_columns):
            await update.message.reply_text("❌ فشل التحديث: بنية ملف الـ Excel غير مطابقة للمواصفات المطلوبة.")
            os.remove(file_path)
            db.close()
            return
        ...
        for index, row in df.iterrows():
            ...
            # البحث إذا كان الموظف مسجل مسبقاً لتحديث بياناته
            ...
            if existing_emp:
                ...
                # إضافة موظف جديد بالكامل
                new_emp = Employee(
                    id=emp_id, name=emp_name, phone_number=emp_phone, role=emp_role,
                    encrypted_salary=encrypt_data("1000.0") # راتب افتراضي مشفر
                )
                db.add(new_emp)
                inserted_count += 1
                
        db.commit()
        await update.message.reply_text(f"✅ **اكتمل تحديث النظام بنجاح!**\n\n🔄 الموظفين المحدثين: {updated_count}\n📥 الموظفين الجدد المضافين: {inserted_count}")
        os.remove(file_path)
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # حماية عامة من أزرار الرجوع لقوائم التصفح الرئيسي
    ...
    if query.data.startswith("emp_"):
        if chat_id not in sessions or "id" not in sessions[chat_id]:
            await context.bot.send_message(
                chat_id=chat_id,
                text="⚠️ **انتهت صلاحية الجلسة الحالية بسبب تحديث النظام.**\n\nيرجى كتابة **رقم هاتفك** مجدداً لإعادة المصادقة الفورية."
            )
            sessions[chat_id] = {"step": "ASK_PHONE"}
            db.close()
            return
        ...
        employee = db.query(Employee).filter(Employee.id == sessions[chat_id]["id"]).first()
        if not employee:
            await context.bot.send_message(chat_id=chat_id, text="❌ لم يتم العثور على بياناتك في النظام.")
            db.close()
            return
        ...
        if query.data == "emp_salary":
            ...
            await query.message.reply_text("📝 اكتب تفاصيل مذكرتك ومطالبك حالياً في رسالة واحدة:")
        ...
        if query.data == "admin_export_import":
            ...
            kb = [[InlineKeyboardButton("↩️ رجوع للقائمة الإدارية", callback_data="back_to_admin_menu")]]
            ...
            if not reqs:
                ...
                await query.message.reply_text("✅ لا توجد طلبات إجازة معلقة حالياً.", reply_markup=InlineKeyboardMarkup(kb))
                db.close()
                return
            for r in reqs:
        ...
        # ✅ تم حل المشكلة: بناء وتفعيل قسم تنظيم المذكرات والشكاوى للمدير
            ...
            if not tickets:
                ...
                await query.message.reply_text("🗂️ لا توجد مذكرات أو شكاوى معلقة بحاجة لمراجعة المالك.", reply_markup=InlineKeyboardMarkup(kb))
                db.close()
                return
    ...
    # معالجة أزرار التفاعل مع الشكاوى والمذكرات
        ...
        if ticket and action == "close":
            ...
            await query.message.reply_text("✅ تم أرشفة الشكوى وتغيير حالتها بنجاح.", reply_markup=InlineKeyboardMarkup(kb))
        ...
        if req:
            ...
            if action == "approve":
                ...
                msg = "✅ تم قبول الطلب وخصم الأيام تلقائياً من الموظف."
            else:
                req.status = "مرفوضة ❌"
...
def main():
    ...
    application.add_handler(MessageHandler(filters.Document.ALL, handle_document)) # معالج قراءة ملفات الإكسل المرفوعة

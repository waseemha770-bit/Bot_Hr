...
# جلب التوكن من المتغيرات البيئية لـ Render (مع توفير التوكن الجديد كقيمة احتياطية)
...
# 🛡️ الدالة الذكية والمطورة بالكامل لقراءة ملفات الإكسل بدون تعارض
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ جاري قراءة ملف الـ Excel وتحديث البيانات بأمان واحترافية...")
    ...
    try:
        ...
        # قراءة الملف مع فرض تحويل جميع الخلايا إلى نصوص لمنع تعارض الحقول
        df = pd.read_excel(file_path, dtype=str)
        
        # تنظيف مسافات العناوين العلوية تلقائياً
        df.columns = [str(col).strip() for col in df.columns]
        
        required_columns = ["الرقم الوظيفي", "الاسم", "رقم الهاتف الدولي", "الصلاحية"]
        if not all(col in df.columns for col in required_columns):
            await update.message.reply_text(
                f"❌ **فشل التحديث:** بنية جدول الإكسل غير مطابقة.\n"
                f"يجب أن يحتوي الملف على الأعمدة التالية تماماً وبنفس الترتيب:\n"
                f"`الرقم الوظيفي` | `الاسم` | `رقم الهاتف الدولي` | `الصلاحية`"
            )
            os.remove(file_path)
            db.close()
            return
        ...
        for index, row in df.iterrows():
            try:
                ...
                raw_phone = str(row["رقم الهاتف الدولي"]).strip()
                ...
                emp_role = str(row["الصلاحية"]).strip() if pd.notna(row["الصلاحية"]) and str(row["الصلاحية"]).strip() != "nan" else "Employee"
                ...
                if existing_emp:
                    ...
                    new_emp = Employee(
                        id=emp_id, name=emp_name, phone_number=emp_phone, 
                        role=emp_role, encrypted_salary=encrypt_data("1000.0")
                    )
                    db.add(new_emp)
                    inserted_count += 1
        ...
        msg = f"✅ **اكتملت معالجة الجدول بنجاح!**\n\n"
        msg += f"📥 الموظفون الجدد المضافون: {inserted_count}\n"
        msg += f"🔄 الحسابات المحدثة: {updated_count}\n"
        if errors_count > 0:
            msg += f"⚠️ أسطر تالفة تم تجاوزها تلقائياً: {errors_count}"
        ...
        await update.message.reply_text("❌ حدث خطأ داخلي غير متوقع أثناء معالجة وقراءة ملف الإكسل.")
    finally:
        db.close()
...
def main():
    ...
    # تشغيل خادم الويب في خلفية السكربت لربط المنفذ بنجاح ومنع تجميد السيرفر
    web_thread = threading.Thread(target=run_web_server, daemon=True)
    web_thread.start()
    
    # بدء اتصال البوت بنظام Polling نقياً وبدون أي تعارض

...
# =====================================================================
# الإعدادات الأساسية وقاعدة البيانات الدائمة (Persistent Database)
# =====================================================================
ADMIN_IDS = [892385625]  # حسابك الثابت والوحيد كمدير عام للنظام
DB_FILE = "database.txt"

# القائمة العامة لتخزين الشكاوى المرسلة وضمان وصولها للوحة المدير
ALL_COMPLAINTS = []

# دالة تحميل الصلاحيات من الملف الدائم لمنع مسح البيانات عند ريستارت السيرفر
...
class DummyWebhookServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.wfile.write(b"HR Enterprise Cloud System is Running!")
...
# =====================================================================
# نظام الحماية بالـ OTP والفصل التام التلقائي بين الموظف والمدير
# =====================================================================
...
async def show_admin_panel(update: Update):
    ...
    await update.message.reply_text(
        ...
        "مرحباً بك يا وسيم. لديك كامل الصلاحيات لإدارة الورديات والموظفين والتحكم في منح وإلغاء صلاحيات الدخول وبشكل دائم ومحفوظ:",
...
async def show_employee_panel(update: Update):
    ...
    await update.message.reply_text("👋 لوحة خدمات الموظف الإلكترونية المؤمنة بالكامل:", reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True))

# =====================================================================
# نظام الشكاوى والاعتراضات الموحد والمثبت (تصل مباشرة للمدير)
# =====================================================================
...
async def send_complaint_preview(target, context, is_callback=False):
    text = (
        ...
        "هل تؤكد الإرسال النهائي للموارد البشرية؟"
    )
    keyboard = [
...
# =====================================================================
# ميزة إدارة الصلاحيات الاحترافية (منح وسحب الصلاحيات وتثبيتها في ملف)
# =====================================================================
...
async def receive_add_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إضافة الموظف الجديد وحفظه في الملف فورياً
    ALLOWED_EMPLOYEES[phone] = name
    save_allowed_employees()
    
    await update.message.reply_text(f"✅ تم منح الصلاحية بنجاح للموظف: *{name}* برقم هاتف: `{phone}` وتم حفظ البيانات بشكل دائم في قاعدة البيانات.", parse_mode="Markdown")
    await show_admin_panel(update)
    return ConversationHandler.END
...
async def start_remove_employee(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    employee_list += "\nالرجاء نسخ وإرسال رقم هاتف الموظف المراد سحب الصلاحية منه وحظره تماماً من النظام:"
...
async def receive_remove_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if phone in ALLOWED_EMPLOYEES:
        ...
        save_allowed_employees() # حفظ الحذف في الملف فوراً
        await update.message.reply_text(f"⛔ تم سحب الصلاحية بنجاح من الموظف: *{emp_name}* وتم حظره وتحديث قاعدة البيانات.", parse_mode="Markdown")
    else:
...
# =====================================================================
# معالجة بقية أزرار الإدارة واستيراد/تصدير Excel الحقيقية
# =====================================================================
async def handle_admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == '📊 إحصائيات الموظفين':
        ...
        if not ALL_COMPLAINTS:
            await update.message.reply_text("📥 لوحة الشكاوى فارغة، لا توجد شكاوى أو اعتراضات مرسلة حالياً من الموظفين.")
            return
        
        await update.message.reply_text(f"📂 **يوجد ( {len(ALL_COMPLAINTS)} ) شكاوى واعتراضات مستلمة من الموظفين:**\n" + "—"*15)
        
        # استعراض كافة الشكاوى المخزنة للمدير وإرسال ملفاتها المرفقة إن وجدت
        for idx, comp in enumerate(ALL_COMPLAINTS, 1):
            comp_msg = (
                f"📌 **الشكوى رقم [ {idx} ]**\n"
                f"👤 **المرسل:** {comp['sender']}\n"
                f"📅 **التاريخ:** {comp['date']}\n"
                f"📝 **نص الاعتراض:**\n_{comp['text']}_"
            )
            await update.message.reply_text(comp_msg, parse_mode="Markdown")
            
            # إذا كانت الشكوى تحتوي على صورة أو مستند مرفق، يرسله البوت للمدير فوراً تحت الشكوى
            if comp['file_id']:
                if comp['file_type'] == 'photo':
                    await update.message.reply_photo(photo=comp['file_id'], caption=f"📎 مرفق الشكوى رقم {idx}")
                else:
                    await update.message.reply_document(document=comp['file_id'], caption=f"📎 مستند الشكوى رقم {idx}")
...
async def handle_all_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "sched_next_week":
        ...
        # جلب اسم الموظف الحالي من الكشوفات لتوثيق الشكوى باسمه الحقيقي
        sender_phone = context.user_data.get('auth_phone', 'unknown')
        sender_name = ALLOWED_EMPLOYEES.get(sender_phone, query.from_user.full_name)
        
        # حفظ الشكوى فعلياً بالكامل داخل المصفوفة العامة المتاحة للمدير
        complaint_object = {
            "sender": sender_name,
            "date": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),
            "text": context.user_data.get('complaint_text', 'بدون نص'),
            "file_id": context.user_data.get('complaint_file'),
            "file_type": context.user_data.get('complaint_file_type')
        }
        ALL_COMPLAINTS.append(complaint_object)
        
        await query.edit_message_text("🚀 **تم إرسال الشكوى بنجاح وأمان إلى الإدارة العليا، وسيتم مراجعتها من قبل المدير فوراً.**")
...
def main():
    ...
    # 5. محادثة حذف وسحب صلاحية موظف للمدير
    ...
    # مستمعات لوحة الإدارة العامة والتحويل والشكاوى المستقرة

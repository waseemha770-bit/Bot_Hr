...
if os.path.exists('./bin/wkhtmltopdf'):
    ...
    # ملاحظة: إذا كان البرنامج مثبتاً في مسار آخر على جهازك قم بتعديله هنا
...
async def generate_employee_report(update: Update, context: ContextTypes.DEFAULT_TYPE, emp_id: str):
    ...
    # بناء جدول HTML متوافق مع اللغة العربية وتنسيق جيد
    html_content = f"""
    <!DOCTYPE html>
    ...
    try:
        # تحويل كود الـ HTML إلى ملف PDF باستخدام الإعدادات الجديدة المتوافقة مع السيرفر
...
def main() -> None:
    # ضع التوكن الخاص بالبوت هنا أو قم بجلبه من متغيرات البيئة

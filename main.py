...
# 🔑 تم تحديث التوكن هنا بناءً على الرمز الجديد لمنع التعارض
TOKEN = "8678088302:AAEN75zakwLfem9Uj55ft9Tc5JeWWWJM9C8"
...
# 🔄 دالات عرض القوائم لتسهيل الرجوع والتصفح بسلاسة
async def show_admin_menu(bot, chat_id):
    ...
    await bot.send_message(
        chat_id=chat_id,
        text="⚙️ **لوحة التحكم الإدارية للموارد البشرية:**\n*(ملاحظة: لتحديث أو رفع بيانات موظفين جديدة، قم بإرسال ملف الـ Excel المعدل مباشرة إلى الشات هنا)*",
...
# 3. معالجات الحوار والرسائل النصية
...
# 📥 نظام الاستيراد والتحديث الذكي عبر رفع ملف Excel للبوت مباشرة
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    db = SessionLocal()
    current_user = db.query(Employee).filter(Employee.telegram_id == str(chat_id)).first()
    
    # حماية: التحقق من صلاحية الإدارة للرفع والتعديل لقاعدة البيانات
    ...
    if not doc.file_name.endswith('.xlsx') and not doc.file_name.endswith('.xls'):
        await update.message.reply_text("❌ يرجى رفع ملف بصيغة Excel الصحيحة فقط (ينتهي بـ .xlsx)")
        db.close()
        return
    ...
    try:
        ...
        # مواءمة الحقول الأساسية المطلوبة
        ...
        for index, row in df.iterrows():
            ...
            if existing_emp:
                ...
                new_emp = Employee(
                    ...
                    encrypted_salary=encrypt_data("1000.0")
                )
                db.add(new_emp)
                inserted_count += 1
...
# 5. معالجة التفاعل مع الأزرار الشاشية
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔄 تفعيل منطق أزرار العودة والرجوع الشاملة
    ...
    if query.data.startswith("emp_"):
        if chat_id not in sessions or "id" not in sessions[chat_id]:
            await context.bot.send_message(
                chat_id=chat_id,
                text="⚠️ **انتهت صلاحية الجلسة الحالية بسبب تحديث النظام الحاصل.**\n\nيرجى كتابة **رقم هاتفك** مجدداً لإعادة التحقق الفوري."
            )
            sessions[chat_id] = {"step": "ASK_PHONE"}
            db.close()
            return
        ...
        if query.data == "emp_salary":
            ...
            await query.message.reply_text("📝 اكتب تفاصيل مذكرتك ومطالبك حالياً في رسالة نصية واحدة ليرفع للمدير:")
        elif query.data == "emp_logout":
            sessions[chat_id] = {"step": "ASK_PHONE"}
            await query.message.reply_text("🔒 تم تسجيل الخروج بنجاح من النظام.", reply_markup=ReplyKeyboardRemove())
        ...
        # ✅ إصلاح وتفعيل زر المذكرات والشكاوى بالكامل للإدارة والمالك
    ...
    # معالجة أزرار إغلاق التذاكر والشكاوى
...
def main():
    ...
    application.add_handler(MessageHandler(filters.Document.ALL, handle_document)) # معالج ملفات الإكسل المرفوعة للتحديث
